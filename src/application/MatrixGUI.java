...
public class MatrixGUI extends Application {
	...
	public void start(Stage stage) {
		...
		multiplyMatrix.setOnAction((event) -> {
			try {
				int i = Integer
						.parseInt(JOptionPane.showInputDialog("Ingrese el número de la matriz A por multiplicar"));
				int j = Integer
						.parseInt(JOptionPane.showInputDialog("Ingrese el número de la matriz B por multiplicar"));
		...
		adjoint.setOnAction((event) -> {
			try {
				int i = Integer
						.parseInt(JOptionPane.showInputDialog("Ingrese el número de la matriz a obtener la adjunta"));
				prepareAnswerFields(resTxtArea, resTxtArea2);
				if (!Matrixes.isSquared(history.get(i))) {
					Alert alert = new Alert(AlertType.ERROR);
					alert.setTitle("Error en las matrices");
		...
		cofactor.setOnAction((event) -> {
			try {
				int i = Integer.parseInt(
						JOptionPane.showInputDialog("Ingrese el número de la matriz a obtener los cofactores"));
				prepareAnswerFields(resTxtArea, resTxtArea2);
				if (!Matrixes.isSquared(history.get(i))) {
					Alert alert = new Alert(AlertType.ERROR);
					alert.setTitle("Error en las matrices");
